...
setTimeout(function(){ 
    ...
    fs.readFile('example_cell_code.js', 'utf8', function (err, data) {
        ...
        env.addCell(new objs.Cell(ENV_SIZE - 300, ENV_SIZE - 300, 80,70, "DemoCell", "Crimson", 1, "hello", code,env.newTeamId(), env.newId()));
        env.addCell(new objs.Cell(ENV_SIZE - 300, 300, 80,70, "DemoCell", "GoldenRod", 1, "hello", code,env.newTeamId(), env.newId()));
        env.addCell(new objs.Cell(300, ENV_SIZE - 300, 80,70, "DemoCell", "MediumSpringGreen", 1, "hello",code,env.newTeamId(), env.newId()));
...
function main() {
    ...
    for (let j = 0; j < env.cells.length; j++) {
        ...
        for (let i = 0; i < env.leeches.length; i++){
            ...
            let cel = env.getCell(leech.owner);
            leech.newpos();
            if(cel){
            if(!cel.bdead && cel.teamid != cell.teamid){
                        b = api.ECI(leech.r, leech.x, leech.y, cellr, cellx, celly);
                        if (b) {
                            cel.heal(leech.energy)
                            env.setCell(cel);
...
function convertenv(env) {
    ...
    env.leeches.forEach(function (leech) {
        pseudoenv.leeches.push(new objs.PseudoLeech(leech.x, leech.y, leech.energy, leech.bdead));
    });
